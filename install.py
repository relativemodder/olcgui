...
STRINGS = {
    'en': {
        ...
        'language_prompt': 'Please choose the language [en/ru]',
    ...
    'ru': {
        ...
        'language_prompt': 'Пожалуйста, выберите язык [ru/en]',
...
def choose_language() -> str:
    while True:
        value = input(f"{STRINGS['en']['choose_lang']} / {STRINGS['ru']['choose_lang']} [en/ru]: ").strip().lower()
        if value in {'en', 'ru'}:
            return value
        print(f"{STRINGS['en']['language_invalid']} / {STRINGS['ru']['language_invalid']}")
...
def render_env(image_prefix: str, image_tag: str, web_port: str, socks_range: str) -> str:
...
def run_command_quiet(cmd: list[str], cwd: Path) -> int:
    proc = subprocess.run(cmd, cwd=str(cwd), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    return proc.returncode
...
def find_compose_tool() -> list[str] | None:
    ...
    for base in candidates:
        ...
        try:
            rc = subprocess.run(check, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL).returncode
...
def deployment_state(compose_dir: Path) -> str:
    ...
    if tool is None:
        return 'missing'
    rc, output = run_command_capture(compose_cmd(tool, compose_dir, ['ps', '-q']), compose_dir)
    if rc != 0:
        return 'missing'
...
def write_deployment_files(compose_dir: Path, image_prefix: str, web_port: str, socks_range: str, selinux: bool) -> None:
    compose_dir.mkdir(parents=True, exist_ok=True)
    compose_file_path(compose_dir).write_text(render_compose(selinux), encoding='utf-8')
    env_file_path(compose_dir).write_text(render_env(image_prefix, DEFAULT_IMAGE_TAG, web_port, socks_range), encoding='utf-8')
...
def show_menu(lang: str, compose_dir: Path) -> None:
    while True:
        ...
        print(f"{tr(lang, 'current_state')}: {tr(lang, f'state_{deployment_kind(compose_dir)}')}")
...
def main() -> int:
    ...
    install_dir = Path(ask(LANG, tr(LANG, 'install_dir'), DEFAULT_INSTALL_DIR)).expanduser().resolve()
    ...
    if compose_path.exists() and not ask_bool(LANG, tr(LANG, 'overwrite_compose', path=str(compose_path)), False):
        print(tr(LANG, 'aborted'))
        return 1
    if env_path.exists() and not ask_bool(LANG, tr(LANG, 'overwrite_env', path=str(env_path)), False):
        print(tr(LANG, 'aborted'))
        return 1

    create_new_deployment(LANG, compose_dir, web_port, socks_range, image_prefix, selinux)
