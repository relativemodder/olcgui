...
function publishInstanceStatus(id: number, status: string, autoRestart: boolean) {
	broker.publish(instanceStatusTopic(id), { id, status, autoRestart });
}
...
export async function startInstance(id: number): Promise<void> {
	...
	if (!existsSync(BINARY_PATH)) {
		...
		publishInstanceStatus(id, 'error', instance.autoRestart);
	...
	publishInstanceStatus(id, 'running', instance.autoRestart);
	...
	(async () => {
		...
		if (restartDecision.shouldRestart) {
			...
			block.autoRestart = false;
			await db.update(instances).set({ status: 'error' }).where(eq(instances.id, id));
			publishInstanceStatus(id, 'error', autoRestart);
			await logInstanceLine(
				id,
...
export async function stopInstance(id: number): Promise<void> {
	...
	await db.update(instances).set({ status: 'stopped' }).where(eq(instances.id, id));
	publishInstanceStatus(id, 'stopped', block.autoRestart);
}
...
export async function restartInstance(id: number): Promise<void> {
	...
	block.status = 'restarting';
	publishInstanceStatus(id, 'restarting', block.autoRestart);
	block.expectedExit = true;
	...
	try {
		...
		if (activeInstances.get(id) === block) {
			block.status = 'error';
			publishInstanceStatus(id, 'error', block.autoRestart);
		}
		throw error;
	}
}
...
async function logInstanceLine(instanceId: number, line: string): Promise<void> {
	...
	broker.publish(instanceLogTopic(instanceId), { instanceId, logLine: line });
	...
	try {
		...
		for (const inst of runningInstances) {
			if (inst.autoRestart) {
				...
				publishInstanceStatus(inst.id, 'stopped', inst.autoRestart);
			}
		}
	} catch (error) {
